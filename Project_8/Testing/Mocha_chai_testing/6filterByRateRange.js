...
var typeOfProduct
var productName
describe('6. filter by rate range',function(){
	...
	it('Count  : should fetch 1333 documents',function(){
		const result = noOfDocuments
		expect(result).to.equal(1333)
	})
  ...
  it('Value  : product name should be equal to CLASSIC',function(){
		const result = productName.split(" ")[0]
		expect(result).to.equal("CLASSIC")
	})
